...
;; Ruby hooks
(ba-add-path ".emacs.d/modes/ruby")
(ba-add-path ".emacs.d/snippet/")
(ba-add-path ".emacs.d/testing")
(setq auto-mode-alist
      (append '(("Rakefile" . ruby-mode)      ;; Rakefiles are just ruby-files
		("\\.rake$" . ruby-mode)
		("\\.gemspec$" . ruby-mode))  ;; same goes for gemspec
	      auto-mode-alist))
(add-hook 'ruby-mode-hook (lambda ()
                            ;; ri support
                            (setq ri-ruby-script
                                  (concat emacs-root "/.emacs.d/modes/ruby/ri-emacs.rb"))
                            (autoload 'ri "~/.emacs.d/modes/ruby/ri-ruby.el")
			    (local-set-key [f1] 'ri)
